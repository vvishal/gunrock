...
#include <fstream>
#include <mutex>
static std::mutex sendRecvMutex;

#define HEADER_MPI_TAG 32767
#define MEMFACTOR 1
#define TAGFACTOR 700
...
void MPI_PushNeighbor(
                      ...
                      void *** mpi_sendbuffer,
                      int batch_size = 500000000,
                      int max_nr_message = 10000
                      )
{
    if (peer == gpu) return;
    int peer_ = peer<gpu? peer+1 : peer;
    void ** sendbuffer = mpi_sendbuffer[original_peer];
    sendbuffer[0] = calloc(8,sizeof(int));
    int * header  = (int*)mpi_sendbuffer[original_peer][0];

    header[0] = gpu;
    header[1] = original_peer;
    header[2] = iteration;
    header[3] = queue_length;
    header[4] = ( (gpu+original_peer)*(gpu+original_peer+1)/2+gpu) % 32000; //Cantor pairing function as a simple hash of a unique tag
    header[5] = batch_size;
    header[6] = ((queue_length-1) / batch_size)+1;  //the number of send commands that have to be issued for a queue
    if(max_nr_message < header[6]) header[6]=max_nr_message;

    sent_requests[gpu][original_peer]=(MPI_Request*)malloc(sizeof(MPI_Request)*(1+header[6]*(num_vertex_associate+num_value__associate)));

    {
        std::lock_guard<std::mutex> lock(sendRecvMutex);
        if(MPI_SUCCESS != MPI_Isend(mpi_sendbuffer[original_peer][0], 7, MPI_INT, peer_server_idx, HEADER_MPI_TAG, MPI_COMM_WORLD,&(sent_requests[gpu][original_peer][0])))
        {
            printf("Unable to sent header to server %i\n", peer_server_idx);
            return;
        }
    }
	//printf("use sent_requests[%i][%i][%i] for header [%i %i %i %i %i %i %i]\n",gpu,original_peer,0,header[0],header[1],header[2],header[3],header[4],header[3],header[4]);



    int sent_flag_counter=1;
    int sendbuffer_counter=1;


    if(queue_length>0)
    {

        sendbuffer[sent_flag_counter] = (VertexId*)calloc(MEMFACTOR*queue_length,sizeof(VertexId));
        ...
        for(int sent_counter=0; sent_counter<header[6]; sent_counter++)
        {
            int nr_elems = (sent_counter+1)*batch_size < queue_length ? batch_size : queue_length % batch_size;
            int tag =header[4] + sent_flag_counter*TAGFACTOR;
            {
                std::lock_guard<std::mutex> lock(sendRecvMutex);
                if(MPI_SUCCESS != MPI_Isend(
                                            sendbuffer[1]+sent_counter*batch_size,//data_slice_l -> keys_out[peer_].GetPointer(util::DEVICE),
                                            nr_elems*sizeof(VertexId),
                                            MPI_BYTE,//mpi_vertex_type,
                                            peer_server_idx,
                                            tag,
                                            MPI_COMM_WORLD,
                                            &(sent_requests[gpu][original_peer][sent_flag_counter])))
                {
        ...
        for (int i=0;i<num_vertex_associate;i++) //TODO FIX VERTICES
        {
            sendbuffer[sendbuffer_counter] = (VertexId*)calloc(queue_length,sizeof(VertexId));
            cudaError_t rv = cudaMemcpy(sendbuffer[sendbuffer_counter],data_slice_l->vertex_associate_outs[peer_][i],queue_length*sizeof(VertexId),cudaMemcpyDeviceToHost);
            ...
            for(int sent_counter=0; sent_counter<header[6]; sent_counter++)
            {
                ...
                int tag =header[4] + sent_flag_counter*TAGFACTOR;
                {
                    std::lock_guard<std::mutex> lock(sendRecvMutex);
                    if(MPI_SUCCESS != MPI_Isend(
                                                sendbuffer[sendbuffer_counter]+sent_counter*batch_size,//data_slice_l->vertex_associate_outs[peer_][i],
                                                nr_elems*sizeof(VertexId),
                                                MPI_BYTE,//mpi_vertex_type,
                                                peer_server_idx,
                                                tag,
        ...
        {
            sendbuffer[sendbuffer_counter] = (Value*)calloc(MEMFACTOR*queue_length,sizeof(Value));
            cudaError_t rv = cudaMemcpy(sendbuffer[sendbuffer_counter],data_slice_l->value__associate_outs[peer_][j],queue_length*sizeof(Value),cudaMemcpyDeviceToHost);
            if (rv != cudaSuccess){
                printf("ERROR in cudaMemcpy in %s:%i\n",__FILE__,__LINE__);
            }
            for(int sent_counter=0; sent_counter<header[6]; sent_counter++)
            {
                int nr_elems = (sent_counter+1)*batch_size < queue_length ? batch_size : queue_length % batch_size;
                int tag =header[4] + sent_flag_counter*TAGFACTOR;
                {
                    std::lock_guard<std::mutex> lock(sendRecvMutex);
                    if (MPI_SUCCESS != MPI_Isend(
                                                sendbuffer[sendbuffer_counter]+sent_counter*batch_size,//data_slice_l->value__associate_outs[peer_][j],
                                                nr_elems*sizeof(Value),
        ...
        header[7] = sendbuffer_counter;
    }else{
        header[7] = 0;  //to indicate that we don't send any message, just the header
    }
...
int MPI_Communication_Check(MPI_Request *** sent_requests, int ** size, int gpu, int peer, int num_gpus_global, void *** sendbuffer)
{
    MPI_Status status;
    int flag=1;
    int num_flags = size[gpu][peer];
    if(peer > num_gpus_global) return 0;

	/*
    {
        std::lock_guard<std::mutex> lock(sendRecvMutex);
        int err = MPI_Test(&(sent_requests[gpu][peer][0]), &flag, &status);
        int * header = (int*)sendbuffer[peer][0];
        printf("check sent_requests[%i][%i][0] = [%i %i %i %i %i], sent flag %i, TestReturnValue %i\n",gpu,peer,
            header[0],header[1],header[2],header[3],header[4],flag, err);
    }
	*/
	
    for(int f=0; f<num_flags; f++){
        std::lock_guard<std::mutex> lock(sendRecvMutex);
        if(MPI_SUCCESS != MPI_Test(&(sent_requests[gpu][peer][f]), &flag, &status)){
            printf("cannot check send request");
        }
        if(flag==0) return 0;
    }
    ...
    int * header = (int*)sendbuffer[peer][0];
    int nr_sendbuffer = header[7];
    for(int i=0;nr_sendbuffer; i++){
...
int MPI_Communication_Wait(MPI_Request *** sent_requests, int ** size, int gpu, int peer, int num_gpus_global, void *** sendbuffer)
{
    MPI_Status status;
    ...
    for(int f=0; f<num_flags; f++){
        ...
        std::lock_guard<std::mutex> lock(sendRecvMutex);
        if(MPI_SUCCESS != MPI_Wait(&(sent_requests[gpu][peer][f]),  &status))
		{
            printf("cannot wait on request[%i][%i][%i]\n",gpu,peer,f);
        }
        int * header = (int*)sendbuffer[peer][0];
    }
    free(sent_requests[gpu][peer]);

    int * header = (int*)sendbuffer[peer][0];
    int nr_sendbuffer = header[7];
    for(int i=0; i<nr_sendbuffer; i++){
        free(sendbuffer[peer][i]);
    }
    return 1;
}
                   ...
                   ThreadSlice *thread_data)
{
    ...
    struct gunrock::app::MPI_Ring_Buffer<SizeT, VertexId, Value> ** all_mpi_ring_buffers =  problem -> mpi_ring_buffer;
    ...
    *s_data_slice         =   problem     -> data_slices;
    thread_data->stats = 1;
    while(thread_data->stats ==1){sleep(0);}

    int final_run=0;


    int all_done = Iteration::Stop_Condition(s_enactor_stats, s_frontier_attribute, s_data_slice, problem -> mpi_topology);
    for(int g=0; g<num_gpu_local; g++)
    {
        all_mpi_ring_buffers[g]->all_done=all_done;
    }

    //synchronize with Iteration_Loop
    ...
    while(!threads_released){
        threads_released=1;
        for(int g=0;g<num_gpu_local; g++){
            if(all_mpi_ring_buffers[g]->checked==3)
            {
                threads_released=0;
            }
        }
    ...
    {
        all_done = Iteration::Stop_Condition(s_enactor_stats, s_frontier_attribute, s_data_slice, problem -> mpi_topology);
        for(int g=0; g<num_gpu_local; g++)
        {
            all_mpi_ring_buffers[g]->all_done=all_done;
        }
        ...
        for(int g=0; g<num_gpu_local; g++)
        {
            all_mpi_ring_buffers[g]->checked=2;
        }
        ...
        while(!threads_released && !all_done){
            threads_released=1;
            for(int g=0;g<num_gpu_local; g++){
                if(all_mpi_ring_buffers[g]->checked==2)
                {
                    threads_released=0;
                }
            }
        ...
        while(recv_counter!=0)
        {
            ...
            MPI_Request header_receiving;

            if(Enactor::DEBUG) printf(" MPI_COMM_LOOP: waiting for %i further mesages\n",recv_counter);
            {
                std::lock_guard<std::mutex> lock(sendRecvMutex);
                int mpi_ret_val = MPI_Irecv(&header[0], header_length, MPI_INT, MPI_ANY_SOURCE, HEADER_MPI_TAG, MPI_COMM_WORLD,&header_receiving);
            ...
            while(!received)
            {
                ...
                {
                    std::lock_guard<std::mutex> lock(sendRecvMutex);
                    MPI_Test(&header_receiving, &received, &status);
                    if(received && Enactor::DEBUG){ printf("received header in MPI_COMM_LOOP\n");}
                }
            }
            ...
            int target_gpu      = header[1];
            //int sender_iter     = header[2];
            int sender_msg_len  = header[3];
            int tag_base        = header[4];
            int batch_size      = header[5];
            int nr_messages     = header[6];


            //header successfully received
            if(Enactor::DEBUG)
            {
                printf("Incoming message from rank %i: [from gpu %i, for gpu %i, iter %i, length %i, tag %i, batch size %i, nr of Messages %i] \n",sender_rank, header[0], header[1], header[2], header[3], header[4], header[5], header[6]);
            }
            ...
            if(sender_msg_len>0)
            {
                ...
                if(mpi_ring_buffer->allocated[sender_gpu][incoming_ring_buffer_pos]==0){
                    //allocate buffer
                    mpi_ring_buffer->buffer_keys[sender_gpu][incoming_ring_buffer_pos]=(VertexId *)calloc(MEMFACTOR*sender_msg_len,sizeof(VertexId));
                    if(mpi_ring_buffer->buffer_keys[sender_gpu][incoming_ring_buffer_pos]==NULL)
                    {
                        fprintf(stderr,"Not enough memory. Aborting . %s:%d\n",__FILE__,__LINE__);
                    }

                    mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos]
                    = (VertexId *)calloc(MEMFACTOR*sender_msg_len * mpi_ring_buffer->num_vertex_associate,sizeof(VertexId));
                    if(mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos]==NULL)
                    {
                        fprintf(stderr,"Not enough memory. Aborting . %s:%d\n",__FILE__,__LINE__);
                    }

                    //printf("reallocating %i bytes\n",sizeof(Value) * sender_msg_len * mpi_ring_buffer->num_value__associate);
                    mpi_ring_buffer->buffer_value[sender_gpu][incoming_ring_buffer_pos]
                    = (Value *)calloc(MEMFACTOR*sender_msg_len * mpi_ring_buffer->num_value__associate,sizeof(Value));
                ...
                if(sender_msg_len > mpi_ring_buffer->allocated[sender_gpu][incoming_ring_buffer_pos])
				{
                    //reallocate buffer
                    ...
                    if(mpi_ring_buffer->num_vertex_associate>0) free(mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos]);
                    mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos] = (VertexId *)calloc(MEMFACTOR*sender_msg_len * mpi_ring_buffer->num_vertex_associate,sizeof(VertexId));
                    if(mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos]==NULL)
                    {
                        fprintf(stderr,"Not enough memory. Aborting . %s:%d\n",__FILE__,__LINE__);
                    }
                    ...
                    if(mpi_ring_buffer->num_value__associate>0) free(mpi_ring_buffer->buffer_value[sender_gpu][incoming_ring_buffer_pos]);
                    mpi_ring_buffer->buffer_value[sender_gpu][incoming_ring_buffer_pos]=(Value *)calloc(MEMFACTOR*sender_msg_len * mpi_ring_buffer->num_value__associate, sizeof(Value));
                    if(mpi_ring_buffer->buffer_value[sender_gpu][incoming_ring_buffer_pos]==NULL)
                    {
                        fprintf(stderr,"Not enough memory. Aborting . %s:%d\n",__FILE__,__LINE__);
                    }
                ...
                MPI_Request key_receiving[nr_messages];
                MPI_Request vertex_receiving[all_mpi_ring_buffers[0]->num_vertex_associate][nr_messages];
                MPI_Request value_receiving[all_mpi_ring_buffers[0]->num_value__associate][nr_messages];

                int tag_addition=1;
                for(int sent_counter=0; sent_counter<nr_messages; sent_counter++)
                {
                    int nr_elems = (sent_counter+1)*batch_size < sender_msg_len ? batch_size : sender_msg_len % batch_size;
                    int tag = tag_base + tag_addition*TAGFACTOR;

                    if(Enactor::DEBUG)
                    {
                        printf("expecting %i bytes from rank %i with tag %i ( %i keys) \n",nr_elems* sizeof(VertexId), sender_rank, tag, nr_elems);
                    }
                    {
                        std::lock_guard<std::mutex> lock(sendRecvMutex);
                        if(MPI_SUCCESS != MPI_Irecv((mpi_ring_buffer->buffer_keys[sender_gpu][incoming_ring_buffer_pos]+sent_counter*batch_size), nr_elems* sizeof(VertexId),MPI_BYTE, sender_rank, tag, MPI_COMM_WORLD, &(key_receiving[sent_counter])))
                        {
                            fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (header)\n");
                            return;
                        }
                ...
                for(int i=0;i<mpi_ring_buffer->num_vertex_associate; i++){
                    for(int sent_counter=0; sent_counter<nr_messages; sent_counter++)
                    {
                        ...
                        {
                            std::lock_guard<std::mutex> lock(sendRecvMutex);
                            if(MPI_SUCCESS != MPI_Irecv(mpi_ring_buffer->buffer_vertex[sender_gpu][incoming_ring_buffer_pos]+offset+sent_counter*batch_size, sizeof(VertexId) * nr_elems, MPI_BYTE , tag, sender_gpu, MPI_COMM_WORLD, &(vertex_receiving[i][sent_counter])))
                            {
                                fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (vertex)\n");
                                return;
                            }
                        }
                    }
                ...
                for(int j=0;j<mpi_ring_buffer->num_value__associate; j++){
                    for(int sent_counter=0; sent_counter<nr_messages; sent_counter++)
                    {
                        ...
                        if(Enactor::DEBUG)
                        {
                            printf("expecting %i bytes from rank %i with tag %i ( %i values) \n",nr_elems* sizeof(VertexId), sender_rank, tag, nr_elems);
                        }
                        {
                            std::lock_guard<std::mutex> lock(sendRecvMutex);
                            if(MPI_SUCCESS != MPI_Irecv((mpi_ring_buffer->buffer_value[sender_gpu][incoming_ring_buffer_pos]+offset+sent_counter*batch_size), sizeof(Value) * nr_elems, MPI_BYTE, sender_rank, tag, MPI_COMM_WORLD, &(value_receiving[j][sent_counter])))
                            {
                                fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (header)\n");
                                return;
                            }
                ...
                if(Enactor::DEBUG) printf("waiting for finalizing the receiving\n");
                received = 0;
                while(!received){
                    received=1;
                    for(int sent_counter=0;sent_counter<nr_messages; sent_counter++)
                    {
                        ...
                        int flag;
                        MPI_Status status;
                        if(MPI_SUCCESS != MPI_Test(&(key_receiving[sent_counter]),&flag,&status)) {fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (keys)\n");return;}
                        if(!flag) received=0;
                    }
                    sleep(0);
                }
                ...
                for(int i=0;i<mpi_ring_buffer->num_vertex_associate; i++)
                {
                    ...
                    while(!received){
                        ...
                        for(int sent_counter=0;sent_counter<nr_messages; sent_counter++)
                        {
                            ...
                            int flag;
                            MPI_Status status;
                            if(MPI_SUCCESS != MPI_Test(&(vertex_receiving[i][sent_counter]),&flag,&status)) {fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (vertex %i)\n",i);return;}
                            if(!flag) received=0;
                        }
                        sleep(0);
                    }
                ...
                for(int j=0;j<mpi_ring_buffer->num_value__associate; j++)
                {
                    ...
                    while(!received){
                        ...
                        for(int sent_counter=0;sent_counter<nr_messages; sent_counter++)
                        {
                            ...
                            int flag;
                            MPI_Status status;
                            if(MPI_SUCCESS != MPI_Test(&(value_receiving[j][sent_counter]),&flag,&status)) {fprintf(stderr,"MPI_Recv error in MPI_Comm_Loop (value %i)\n",j);return;}
                            if(!flag) received=0;
                        }
                        sleep(0);
                    }
            ...
            if(Enactor::DEBUG)
            {
                printf("receiving of message with tag %i successful \n",tag_base);
            }
            mpi_ring_buffer->front_pos[sender_gpu] = incoming_ring_buffer_pos_new;
            mpi_ring_buffer->fill_level[sender_gpu]++;
            if(Enactor::DEBUG){
                char fill_levels[num_gpus_local*(3+problem->mpi_topology->total_num_gpus)+1];
                int pos=0;
                for(int l=0; l<num_gpus_local; l++){
                    fill_levels[pos]='[';
                    pos++;
                    for(int i=0;i<problem->mpi_topology->total_num_gpus; i++)
                    {
                        fill_levels[pos]='0'+all_mpi_ring_buffers[l]->fill_level[i];
                        pos++;
                    }
                    fill_levels[pos]=']';
                    pos++;
                ...
                fill_levels[num_gpus_local*(3+problem->mpi_topology->total_num_gpus)]='\0';
                printf("new fill levels after adding to %i (local %i) = [%s]\n",target_gpu,problem->mpi_topology->local_gpu[target_gpu],fill_levels);
            }
        ...
        //wait until the other threads are all done with the receiving part
        for(int g=0;g<num_gpu_local; g++){
            all_mpi_ring_buffers[g]->checked=3;
        }
        threads_released=0;
    ...
    for(int g=0; g<num_gpu_local; g++)
    {
        all_mpi_ring_buffers[g]->all_done=2;
    }
}
                                 ...
                                 cudaStream_t     stream)
{
    ...
    bool to_reallocate=false;
    if (target_data_slice -> keys_in[t][peer_].GetSize() < queue_length) to_reallocate=true;
    else {
        for (i=0;i<num_vertex_associate;i++)
            if (target_data_slice->vertex_associate_in[t][peer_][i].GetSize() < queue_length) {to_reallocate=true;break;}
        if (!to_reallocate)
            for (i=0;i<num_value__associate;i++)
                if (target_data_slice->value__associate_in[t][peer_][i].GetSize() < queue_length) {to_reallocate=true;break;}
    }
    ...
    if (to_reallocate)
    {
        // check size for keys
        if (enactor_stats->retval = Check_Size<SIZE_CHECK, SizeT, VertexId>(
                                                                            "keys_in", queue_length, &target_data_slice -> keys_in[t][peer_], over_sized,
                                                                            peer_, enactor_stats->iteration, peer)) return;

        //check size for vertex associates
        for (int i=0;i<num_vertex_associate;i++)
        {
            if (enactor_stats->retval = Check_Size<SIZE_CHECK, SizeT, VertexId>(
                                                                                "vertex_associate_in", queue_length, &target_data_slice->vertex_associate_in[t][peer_][i], over_sized,
                                                                                peer_, enactor_stats->iteration, peer)) return;
            target_data_slice->vertex_associate_ins[t][peer_][i] = target_data_slice->vertex_associate_in[t][peer_][i].GetPointer(util::DEVICE);
        ...
        {
            if (enactor_stats->retval = Check_Size<SIZE_CHECK, SizeT, Value>(
                                                                             "value__associate_in", queue_length, &target_data_slice->value__associate_in[t][peer_][j], over_sized,
                                                                             peer_, enactor_stats->iteration, peer)) return;
            target_data_slice->value__associate_ins[t][peer_][j] = target_data_slice->value__associate_in[t][peer_][j].GetPointer(util::DEVICE);
        }

        if (SIZE_CHECK)
        {
            if (enactor_stats->retval = target_data_slice->vertex_associate_ins[t][peer_].Move(util::HOST, util::DEVICE)) return;
            if (enactor_stats->retval = target_data_slice->value__associate_ins[t][peer_].Move(util::HOST, util::DEVICE)) return;
        }
    }
                    ...
                    ThreadSlice *thread_data)
{
    ...
    int num_gpu_global                 =  problem -> mpi_topology -> total_num_gpus;
    int gpu                            =  problem -> mpi_topology -> global_gpu_maping[rank][thread_data -> thread_num];
    struct gunrock::app::MPI_Ring_Buffer<SizeT, VertexId, Value> * mpi_ring_buffer =  problem -> mpi_ring_buffer[thread_data -> thread_num];
#else
    int rank                           =  0;
    ...
    util::Array1D<SizeT, DataSlice>
    *s_data_slice         =   problem     -> data_slices;
    GraphSlice   *graph_slice          =   problem     -> graph_slices       [local_gpu] ;
    GraphSlice   **s_graph_slice       =   problem     -> graph_slices;
    FrontierAttribute<SizeT>
    *frontier_attribute   = &(enactor     -> frontier_attribute [thread_data -> thread_num * num_gpu_global]);
    FrontierAttribute<SizeT>
    *s_frontier_attribute = &(enactor     -> frontier_attribute [0         ]);
    EnactorStats *enactor_stats        = &(enactor     -> enactor_stats      [thread_data -> thread_num * num_gpu_global]);
    EnactorStats *s_enactor_stats      = &(enactor     -> enactor_stats      [0         ]);
    util::CtaWorkProgressLifetime
    *work_progress        = &(enactor     -> work_progress      [thread_data -> thread_num * num_gpu_global]);
    ...
    *scanned_edges_       =   NULL;
...
#ifdef WITHMPI
    printf("Iteration entered (rank %i, gpu %i)\n",problem -> mpi_topology->local_rank,gpu);fflush(stdout);
    void *** mpi_sendbuffers = (void***)calloc(num_gpu_global,sizeof(void**));
    MPI_Request ***sent_requests = (MPI_Request ***)calloc(num_gpu_global,sizeof(MPI_Request **));

    int **sent_request_size = (int**)calloc(MEMFACTOR*num_gpu_global,sizeof(int*));
    ...
    {
        sent_requests[g]     = (MPI_Request **)calloc(num_gpu_global,sizeof(MPI_Request*));
        sent_request_size[g] = (int*)calloc(num_gpu_global, sizeof(int*));
        mpi_sendbuffers[g]   = (void**)calloc(4,sizeof(void*));  //4 buffers: header,keys, vertex_associates, value_associates
        if(sent_requests[g]==NULL || sent_request_size[g]==NULL)
        {
            printf("Out of memory\n");
            return;
        }
    ...
    while(mpi_ring_buffer->checked==0){sleep(0);}
    mpi_ring_buffer->checked=0;
    while (!mpi_ring_buffer->all_done)
...
#endif
        {
            ...
            while (data_slice->wait_counter < num_gpu_global*2 && !mpi_ring_buffer->all_done)
#else
                            ...
                            case 0: // Assign marker & Scan and send & receive
                                ...
                                if (peer_==0) {
                                    ...
                                    if(problem->mpi_topology->rank_of_gpu[original_peer] == problem->mpi_topology->rank_of_gpu[gpu])
                                    {
                                        ...
                                        > (gpu,
                                           ...
                                           original_peer,
                                           iteration,
                                           problem->mpi_topology->rank_of_gpu[original_peer],
                                           0,
                                           enactor_stats_,
                                           ...
                                           mpi_sendbuffers,
                                           problem->mpi_topology->max_send_buffer_size,
                                           problem->mpi_topology->max_nr_messages
                                           );
                                    }
                                    stages[peer__]=3;
                                    break;
                                ...
                                { //wait and expand incoming
                                    ...
                                    if(problem->mpi_topology->rank_of_gpu[original_peer] != problem->mpi_topology->rank_of_gpu[gpu])  //if data was received via MPI from another node
                                    {
                                        ...
                                        {//if not, do nothing
                                            char fill_levels[num_gpu_global];
                                            for(i=0;i<num_gpu_global; i++)
                                            {
                                                fill_levels[i]='0'+mpi_ring_buffer->fill_level[i];
                                            }
                                            if(0 && Enactor::DEBUG){
                                                printf(" gpu/peer__ %i/%i : checked for %d. Fill level = [%s]\n",gpu,peer__,original_peer, fill_levels);
                                                usleep(500);
                                            }
                                            to_show[peer__]=false;
                                            stages[peer__]--;        //at the end ofthe loop, the stages increase. Hence "--" means to stay in stage 0
                                            //printf("no data for gpu %i from original_peer %i.\n",gpu,original_peer);
                                    ...
                                    else   //data was received via CudaMemcpy
                                    {
                                        //set via
                                        //
                                        //data_slice->events[iteration%4][peer_][stage]
                                        //on the sending gpu
                                        //
                                        if (!(s_data_slice[local_peer]->events_set[iteration_][gpu_][0])) //check if event happened
                                        {   //if not, do nothing
                                        ...
                                        //reset event flag
                                        s_data_slice[local_peer]->events_set[iteration_][gpu_][0]=false;
            ...
            if (!mpi_ring_buffer->all_done)
...
#endif
                {
                    ...
                    wait_count=0;
#ifdef WITHMPI
                    while (wait_count<num_gpu_global*2-1 && !mpi_ring_buffer->all_done)
...
#endif
                        {
                            for (peer_=0;peer_<num_gpu_global*2;peer_++)
                            {
                                ...
                                {
                                    if(peer_<num_gpu_global){
                                        wait_count++;
                                        continue;  //MPI receive is done in MPI_Comm_Loop, no Communication Check necessary
                                    }
                                    if (MPI_Communication_Wait(sent_requests, sent_request_size, gpu, original_peer, num_gpu_global, mpi_sendbuffers))   //check if MPI_Sent or receive for peer_ is completed
                                    {
                    ...
                    while(mpi_ring_buffer->checked!=3){sleep(0);}
                    mpi_ring_buffer->checked=0;
...
/*
    for(int i=0;i<num_gpu_global; i++)
    {
        for(int p=0;p<num_gpu_global; p++){
            free(sent_requests[i][p]);
        }
        free(sent_request_size[i]);
        free(sent_requests[i]);
        free(mpi_sendbuffers[i]);
    }
...
{
    int maxsentbuffer = 1<<31;
    int maxmessages = 1<<31;
    ...
    if(conf_file.is_open())
    {
        ...
        while(conf_file.good())
        {
            ...
            if(token.size() > 1){
                ...
                {
                    if(token[0]=="#PARAM" && token.size() > 2) //parameter for MPI compututation
                    {
                        if (token[1]=="MAXSENTBUFFER") maxsentbuffer = atoi(token[2].c_str());
                        if (token[1]=="MAXSENTMESSAGES") maxmessages = atoi(token[2].c_str());
                    }
                }
            }
        }
    ...
    topol.num_gpus_per_server  = (int  *)calloc(topol.num_servers,sizeof(int));
    topol.rank_of_gpu          = (int  *)calloc(global_gpu_nr,sizeof(int));
    topol.local_gpu            = (int  *)calloc(global_gpu_nr,sizeof(int));
    topol.local_gpu_mapping    = (int **)calloc(topol.num_servers,sizeof(int*));
    topol.global_gpu_maping    = (int **)calloc(topol.num_servers,sizeof(int*));
    topol.global_to_device_idx = (int  *)calloc(global_gpu_nr,sizeof(int));
    topol.total_num_gpus       = global_gpu_nr;
    topol.max_send_buffer_size = maxsentbuffer;
    topol.max_nr_messages      = maxmessages;

    int offset=0;
    for(int i=0;i<topol.num_servers; i++)
    {
        topol.num_gpus_per_server[i] = (int)gpu_mapping[i].size();
        topol.local_gpu_mapping[i]   = (int*)calloc(topol.num_gpus_per_server[i],sizeof(int));
        topol.global_gpu_maping[i]   = (int*)calloc(topol.num_gpus_per_server[i],sizeof(int));
...
protected:
                ...
                )
    {
        ...
        this->num_gpus_local       = num_gpu_local;
        cuda_props        .SetName("cuda_props"        );
        work_progress     .SetName("work_progress"     );
        enactor_stats     .SetName("enactor_stats"     );
        frontier_attribute.SetName("frontier_attribute");
...
#ifdef WITHMPI
            if (retval = util::SetDevice(gpu_idx_local[gpu])) return retval;
#else
